...
const { TaskEventLogStore } = require("./task-event-log-store");
...
function createGateway(config) {
  ...
  function getWarehouseConfig() {
    ...
    const configData = state && state.config && typeof state.config === "object"
      ...
      : FARM_CONFIG_DEFAULT;
    return {
      autoWarehouseSellEnabled: configData.autoWarehouseSellEnabled !== false,
  ...
  function buildWarehouseSchedulerState() {
    ...
    return {
      ...
      autoSellEnabled: cfg.autoWarehouseSellEnabled !== false,
  ...
  async function executeWarehouseRefreshOperation({ reason, allowAutoSell = true } = {}) {
    ...
    try {
      ...
      if (allowAutoSell && cfg.autoWarehouseSellEnabled !== false && candidateIds.length > 0 && isWarehouseAutoSellDue(cfg)) {
        try {
  ...
  const httpServer = http.createServer(async (req, res) => {
    ...
    if (req.method === "GET" && urlPath === "/api/task-logs") {
      try {
        ...
        const data = await taskEventLogStore.readRecent(limit);
        res.writeHead(200, { "Content-Type": "application/json; charset=utf-8" });
        res.end(JSON.stringify({
          ok: true,
          data: {
    ...
    if (req.method === "GET" && urlPath === "/api/task-logs/export") {
      try {
        ...
        const content = format === "json"
          ? await taskEventLogStore.exportJson(limit)
          : await taskEventLogStore.exportText(limit);
