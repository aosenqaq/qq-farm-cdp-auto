...
async function getSeedList(session, callGameCtl) {
  return await callGameCtlWithRecovery(session, callGameCtl, "gameCtl.getSeedList", [withSilent({ sortMode: 3 })], null);
}

async function getShopSeedList(session, callGameCtl) {
  await callGameCtl(session, "gameCtl.requestShopData", [2]);
  return await callGameCtlWithRecovery(session, callGameCtl, "gameCtl.getShopSeedList", [withSilent({ sortByLevel: true })], null);
}
